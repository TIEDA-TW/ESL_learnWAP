...
class BookData:
            ...
            with open(self.json_path, 'r', encoding='utf-8') as f:
                ...
                for element in self.elements:
                    ...
                    if 'id' not in element:
                        ...
                        print(f"給元素 {element.get('Text', 'Unknown')} 添加唯一ID: {element['id']}")
                    # 確保每個元素都有頁面標記
    ...
    def save(self):
        ...
        try:
            ...
            # 保存完畢後，重建 elements 與 pages
            self.elements = elements_to_save
            self.pages = {}
            for element in self.elements:
                if 'Image' in element:
                    page_key = element['Image']
                    if page_key not in self.pages:
                        self.pages[page_key] = []
                    element['Page'] = page_key
                    if 'X1' in element and 'Y1' in element and 'X2' in element and 'Y2' in element:
                    ...
                    self.pages[page_key].append(element)
                else:
                    print(f"警告: 元素缺少頁面標記: {element.get('Text', '未知')}")
            print(f"重建了 {len(self.pages)} 個頁面的數據")
            for page_key, page_data in self.pages.items():
                print(f"頁面 {page_key}: {len(page_data)} 個元素")
            return True
        except Exception as e:
